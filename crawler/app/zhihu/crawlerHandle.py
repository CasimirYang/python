...
def get_aio_proxy():
    ip, host = func.__next__()
    return aiohttp.ProxyConnector("http://{0}:{1}".format(ip, host))
...
async def handle_follow(origin_user):
    print("begin to handle user:{0}".format(origin_user.user_id))
    ...
    while 1:
        item_flag += 1
        try:
            await handle_follow_item(origin_user, 'ee')
    ...
    while 1:
        item_flag += 1
        try:
            await handle_follow_item(origin_user, 'er')
        except Exception as e:
...
async def handle_follow_item(origin_user, follow_type):
    ...
    user_detail = await get_user_detail(homepage)
    ...
    if follow_count != 0:
        ...
        while offset < follow_count:
            ...
            resp = await get_response(follow_url, follow_data, headers)
            soup = BeautifulSoup(str(resp['msg']), "html.parser", from_encoding='UTF-8',
...
async def get_user_detail(homepage):
    resp = await get_response(homepage, None, None, 'get')
    homepage_soup = BeautifulSoup(resp, "html.parser", from_encoding='UTF-8')
...
async def get_response(follow_url, follow_data, headers, request_type='post'):
    retry_flag = 1
    while 1:
        ...
        try:
            if request_type == 'get':
                async with client.get(follow_url, data=follow_data, headers=headers) as resp:
                    content = await resp.text()
            else:
                async with client.post(follow_url, data=follow_data, headers=headers) as resp:
                    content = await resp.json()
            if resp.status != 200:
                logger.error("error status:{0}".format(resp.status))
                #session.proxies = get_proxy()
                client._connector = get_aio_proxy()
            else:
                return content
